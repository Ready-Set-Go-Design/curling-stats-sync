import { CollectionConfig, CsvRow, SyncMode, SyncOperation, SyncResult } from "@/lib/types";
...
function buildFieldData(
  ...
  for (const [csvColumn, fieldSlug] of Object.entries(config.fieldMap)) {
    ...
    if (referenceConfig) {
      ...
      fieldData[fieldSlug] =
        resolvedIds.length === 1 ? resolvedIds[0] : resolvedIds.filter(Boolean);
      continue;
    }
...
async function buildReferenceIndexes(
  token: string,
  config: CollectionConfig
...
export async function syncCollection(params: {
  ...
  const { operations, errors } = planOperations(
    rows,
    config,
    existingItems,
    referenceIndexes,
